// lib/data/lgas.ts
// All 774 Nigerian LGAs, keyed by state. The onboarding LGA field NEVER
// shows this flat -- it calls getLGAsForState(state) so a student only ever
// sees the LGAs for the state of origin they just picked. ALL_LGAS is kept
// for admin/search surfaces only, never for the student-facing combobox.
export const LGAS_BY_STATE: Record<string, string[]> = {
  ...
  "Rivers": ["Abua/Odual","Ahoada East","Ahoada West","Akuku-Toru","Andoni","Asari-Toru","Bonny","Degema","Eleme","Emohua","Etche","Gokana","Ikwerre","Khana","Obio/Akpor","Ogba/Epkrama","Ogu/Bolo","Okrika","Omuma","Opobo/Nkoro","Oyigbo","Port Harcourt","Tai"],
...
// The ONLY accessor the student-facing onboarding should use. Returns the
// LGAs for one state, alphabetically, or [] when the state is unknown so
// the combobox stays disabled rather than falling back to a 774-item list.
export function getLGAsForState(state: string): string[] {
  const list = LGAS_BY_STATE[state];
  if (!list) return [];
  return [...list].sort((a, b) => a.localeCompare(b));
}

// Flat list for admin/search surfaces only. Never wire this into the
// student LGA combobox.
export const ALL_LGAS: string[] = Object.values(LGAS_BY_STATE)
  .flat()
