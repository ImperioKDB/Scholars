...
export async function GET() {
  ...
  const { data: savedRows, error } = await supabase
    ...
    .select("id, saved_at, scholarship_id")
    .eq("profile_id", user.id)
    .order("saved_at", { ascending: false });
  if (error) {
    console.error("saved_scholarships_read_failed", { userId: user.id, error });
    return NextResponse.json(
      { error: "Unable to load saved scholarships" },
      { status: 500 },
    );
  }
  const ids = (savedRows ?? []).map((row) => row.scholarship_id);
  const { data: scholarships, error: scholarshipError } = ids.length
    ? await supabase
        .from("scholarships")
        .select(
    ...
    : { data: [], error: null };
  if (scholarshipError) {
    console.error("saved_scholarship_listing_read_failed", {
      userId: user.id,
  ...
  const data = (savedRows ?? [])
    .map((row) => ({
      ...
      scholarship: byId.get(row.scholarship_id) ?? null,
    }))
    .filter((row) => row.scholarship !== null);
...
export async function DELETE(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      {
        error: "Invalid or missing scholarship_id query param",
