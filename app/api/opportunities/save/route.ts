...
export async function GET() {
  ...
  const { data: savedRows, error } = await supabase
    ...
    .select("id, saved_at, opportunity_id")
    .eq("profile_id", user.id)
    .order("saved_at", { ascending: false });
  if (error) {
    console.error("saved_opportunities_read_failed", {
      userId: user.id,
      error,
    });
    return NextResponse.json(
      { error: "Unable to load saved opportunities" },
      { status: 500 },
    );
  }
  const ids = (savedRows ?? []).map((row) => row.opportunity_id);
  const { data: opportunities, error: opportunityError } = ids.length
    ? await supabase
        .from("opportunities")
    ...
    : { data: [], error: null };
  if (opportunityError) {
    console.error("saved_opportunity_listing_read_failed", {
      userId: user.id,
  ...
  const data = (savedRows ?? [])
    .map((row) => ({
      ...
      opportunity: byId.get(row.opportunity_id) ?? null,
    }))
...
export async function DELETE(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      {
        error: "Invalid or missing opportunity_id query param",
