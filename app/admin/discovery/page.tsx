...
type Candidate = { id: string; source_id: string; source_url: string; application_url: string | null; title: string; provider_name: string; description: string | null; amount: string | null; deadline: string | null; level: string; discipline: string | null; eligibility_notes: string | null; evidence_excerpt: string; fetched_at: string; confidence: number | null; status: string; rejection_reason: string | null; reviewed_at: string | null; duplicate_of: string | null; duplicate_score: number | null; duplicate_reason: string | null; verification_status: string; verification_http_status: number | null; last_verified_at: string | null; quality_status: string; quality_score: number | null; quality_issues: string[]; quality_scored_at: string | null; eligibility_review_status: string; eligibility_verdict: string | null; eligibility_confidence: number | null; eligibility_report: { reasons?: string[]; requirements?: string[]; contradictions?: string[]; evidence_quotes?: string[] } | null; eligibility_reviewed_at: string | null; eligibility_review_error: string | null; created_at: string }
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-xl border border-hairline bg-white p-5">
        ...
        <div className="mt-5 divide-y divide-hairline">{sources.map((source) => <div key={source.id} className="flex flex-col gap-2 py-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium text-navy">{source.name}</p><a href={source.base_url} target="_blank" rel="noreferrer" className="text-xs text-navy-light hover:underline break-all">{source.base_url}</a></div><div className="flex items-center gap-2 text-xs"><span className="rounded-full bg-parchment px-2.5 py-1 text-navy-light">{source.source_type}</span><span className={`rounded-full px-2.5 py-1 ${source.enabled ? 'bg-green-50 text-green-700' : 'bg-amber-light text-amber'}`}>{source.enabled ? 'Enabled' : 'Disabled'}</span><span className={`rounded-full px-2.5 py-1 ${source.pilot_enabled ? 'bg-blue-50 text-blue-700' : 'bg-parchment text-navy-light'}`}>{source.pilot_enabled ? 'Pilot on' : 'Pilot off'}</span></div></div>)}</div>
      </section>
