...
//
// MOTION: `transition` (not transition-all) so only the intended properties
// (color, background, border, transform, opacity) animate -- transition-all
// also interpolates unrelated properties and can jank.
export function BackLink({
  ...
  return (
    <Link
      href={href}
      className="inline-flex items-center gap-2 rounded-seal border border-hairline bg-white px-4 py-2.5 text-sm font-medium text-navy hover:bg-navy-50 hover:border-navy/20 active:scale-[0.98] transition mb-6"
    >
