...
//
// In-app feedback launcher, mounted from the Sidebar so it appears on every
// authenticated student surface. Sits bottom-LEFT, deliberately opposite
// Ade (bottom-right), so the two floating controls never compete for the
// same thumb zone on a phone.
//
...
export function FeedbackWidget() {
  ...
  // Pre-fill the reply address once, lazily, only when the modal opens.
  useEffect(() => {
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-20 md:bottom-4 left-4 z-[90] rounded-seal bg-white border border-hairline shadow-card text-navy-light text-xs font-medium px-4 py-2.5 hover:text-navy hover:border-navy/40 transition-colors"
      >
        Feedback
      </button>
      {open && (
