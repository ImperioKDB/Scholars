...
//
// PHASE 4 OUTCOME: "Download packet (.txt)" bundles the statement + facts
// + checklist into a single plain-text file the student can keep, email
// to a recommender, or paste into any provider portal that accepts a
// text upload. No cloud dependency, no format surprises.
export type Draft = {
...
export function DraftPanel({
  ...
  // Phase 4 outcome: download the draft + summary + checklist as a
  // single .txt file. Filename is slugified from the scholarship title
  // so a student tracking several awards gets files that don't collide.
  function downloadPacket() {
    const facts = draft.draft_summary?.facts.map((f) => `${f.label}: ${f.value}`).join("\n") ?? "";
    const checklist = draft.draft_summary?.checklist
      .map((c) => `${c.have ? "[x]" : "[ ]"} ${c.item}`)
      .join("\n") ?? "";
    const body = [
      `Application draft: ${scholarshipTitle}`,
      `Generated: ${draft.draft_generated_at ?? "unknown"}`,
      `Status: ${confirmed ? "confirmed by student" : "needs review"}`,
      "",
      "=== PERSONAL STATEMENT ===",
      "",
      ...
      "",
      "---",
      "This draft isn't submitted anywhere. Review it, then copy it into",
      "the scholarship's own application page.",
    ].join("\n");

    const blob = new Blob([body], { type: "text/plain;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const slug = scholarshipTitle
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-|-$/g, "")
      .slice(0, 60);
    ...
    a.href = url;
    a.download = `${slug || "application-draft"}.txt`;
